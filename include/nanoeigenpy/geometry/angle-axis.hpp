...
void exposeAngleAxis(nb::module_ m, const char *name) {
  ...
  auto cl =
      nb::class_<AngleAxis>(m, name,
          ...
          .def(nb::init<const Scalar &, Vector3>(), nb::arg("angle"),
               nb::arg("axis"), "Initialize from angle and axis.")
          .def(nb::init<Matrix3>(), nb::arg("R"),
               "Initialize from a rotation matrix.")
          .def(nb::init<Quaternion>(), nb::arg("quaternion"),
               "Initialize from a quaternion.")
          .def(nb::init<AngleAxis>(), nb::arg("copy"), "Copy constructor.")

          .def_prop_rw(
              "angle", [](const AngleAxis &aa) { return aa.angle(); },
              [](AngleAxis &aa, Scalar a) { aa.angle() = a; },
              "The rotation angle.", nb::rv_policy::reference_internal)
          .def_prop_rw(
